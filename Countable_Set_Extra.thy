...
proof -
  ...
  thus ?thesis
    ...
    apply (rule comp_inj_on)
    using cset2infseq_inj inj_on_subset apply auto[1]
    apply (rule comp_inj_on)
     apply (meson UNIV_I infseq_inj inj_on_subset subsetI)
    apply (metis bij_betw_imp_surj_on bit_infseq_of_nat_cset_bij inj_eq inj_on_def surj_imp_inj_inv)
    done
qed
